...
class TableContent(Content):
    ...
    def set_translation(self, translation, status):
        try:
            ...
            LOG.debug(f'when set_translattion ,this is the translation {translation}',translation)
            ...
            for line in lines:
                ...
                # 将每个项目字符串转换为列表
                items_list = [item.strip() for item in items]
                # 将每个项目列表添加到结果列表中
                table_data.append(items_list)

            LOG.debug(f'when parse the table_data ,this is the table_data {table_data}',table_data)
